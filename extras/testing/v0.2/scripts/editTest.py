...
def renderTestOutput(id, data,testCursor):
   ...
   temp += u'<input type="submit" name="sub_del_%s" value="Remove" />'%(prefix[:-1])
   text += divit(temp, "lrel")
   # Mode
   text += divit(renderRadioButton("Compare output against", "%s%s"%(prefix,"mode"), omd, ["Last passed", "Specified", "Autogenerated"]), "lrel")
